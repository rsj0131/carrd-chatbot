...
export default async function handler(req, res) {
    ...
    try {
        ...
        const model = genAI.getGenerativeModel({
            ...
            generationConfig: {
                maxOutputTokens: 4096, // Limit tokens for summarization
                temperature: 0.5,     // Maintain balanced creativity and accuracy
            },
        });
...
async function getAnswer(userQuery) {
    ...
    try {
        ...
        // Log similarity values for all entries
        console.log("Similarity scores for all entries:");
        similarities.forEach(({ entry, similarity }, index) => {
            console.log(`Entry ${index + 1}: Similarity = ${similarity.toFixed(6)}.`);
        });

        // Step 5: Select top N most relevant entries
        const topN = 3; // Adjust this number based on testing and user experience
        const threshold = 0.46; // Adjust this threshold based on desired precision
